// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from rt_interfaces:srv/SolveIK.idl
// generated code does not contain a copyright notice
#include "rt_interfaces/srv/detail/solve_ik__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
rt_interfaces__srv__SolveIK_Request__init(rt_interfaces__srv__SolveIK_Request * msg)
{
  if (!msg) {
    return false;
  }
  // x
  // y
  // z
  return true;
}

void
rt_interfaces__srv__SolveIK_Request__fini(rt_interfaces__srv__SolveIK_Request * msg)
{
  if (!msg) {
    return;
  }
  // x
  // y
  // z
}

bool
rt_interfaces__srv__SolveIK_Request__are_equal(const rt_interfaces__srv__SolveIK_Request * lhs, const rt_interfaces__srv__SolveIK_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // x
  if (lhs->x != rhs->x) {
    return false;
  }
  // y
  if (lhs->y != rhs->y) {
    return false;
  }
  // z
  if (lhs->z != rhs->z) {
    return false;
  }
  return true;
}

bool
rt_interfaces__srv__SolveIK_Request__copy(
  const rt_interfaces__srv__SolveIK_Request * input,
  rt_interfaces__srv__SolveIK_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // x
  output->x = input->x;
  // y
  output->y = input->y;
  // z
  output->z = input->z;
  return true;
}

rt_interfaces__srv__SolveIK_Request *
rt_interfaces__srv__SolveIK_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rt_interfaces__srv__SolveIK_Request * msg = (rt_interfaces__srv__SolveIK_Request *)allocator.allocate(sizeof(rt_interfaces__srv__SolveIK_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rt_interfaces__srv__SolveIK_Request));
  bool success = rt_interfaces__srv__SolveIK_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rt_interfaces__srv__SolveIK_Request__destroy(rt_interfaces__srv__SolveIK_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rt_interfaces__srv__SolveIK_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rt_interfaces__srv__SolveIK_Request__Sequence__init(rt_interfaces__srv__SolveIK_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rt_interfaces__srv__SolveIK_Request * data = NULL;

  if (size) {
    data = (rt_interfaces__srv__SolveIK_Request *)allocator.zero_allocate(size, sizeof(rt_interfaces__srv__SolveIK_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rt_interfaces__srv__SolveIK_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rt_interfaces__srv__SolveIK_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rt_interfaces__srv__SolveIK_Request__Sequence__fini(rt_interfaces__srv__SolveIK_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rt_interfaces__srv__SolveIK_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rt_interfaces__srv__SolveIK_Request__Sequence *
rt_interfaces__srv__SolveIK_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rt_interfaces__srv__SolveIK_Request__Sequence * array = (rt_interfaces__srv__SolveIK_Request__Sequence *)allocator.allocate(sizeof(rt_interfaces__srv__SolveIK_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rt_interfaces__srv__SolveIK_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rt_interfaces__srv__SolveIK_Request__Sequence__destroy(rt_interfaces__srv__SolveIK_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rt_interfaces__srv__SolveIK_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rt_interfaces__srv__SolveIK_Request__Sequence__are_equal(const rt_interfaces__srv__SolveIK_Request__Sequence * lhs, const rt_interfaces__srv__SolveIK_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rt_interfaces__srv__SolveIK_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rt_interfaces__srv__SolveIK_Request__Sequence__copy(
  const rt_interfaces__srv__SolveIK_Request__Sequence * input,
  rt_interfaces__srv__SolveIK_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rt_interfaces__srv__SolveIK_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rt_interfaces__srv__SolveIK_Request * data =
      (rt_interfaces__srv__SolveIK_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rt_interfaces__srv__SolveIK_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rt_interfaces__srv__SolveIK_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rt_interfaces__srv__SolveIK_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
rt_interfaces__srv__SolveIK_Response__init(rt_interfaces__srv__SolveIK_Response * msg)
{
  if (!msg) {
    return false;
  }
  // t1
  // t2
  // t3
  // success
  // mis
  return true;
}

void
rt_interfaces__srv__SolveIK_Response__fini(rt_interfaces__srv__SolveIK_Response * msg)
{
  if (!msg) {
    return;
  }
  // t1
  // t2
  // t3
  // success
  // mis
}

bool
rt_interfaces__srv__SolveIK_Response__are_equal(const rt_interfaces__srv__SolveIK_Response * lhs, const rt_interfaces__srv__SolveIK_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // t1
  if (lhs->t1 != rhs->t1) {
    return false;
  }
  // t2
  if (lhs->t2 != rhs->t2) {
    return false;
  }
  // t3
  if (lhs->t3 != rhs->t3) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // mis
  if (lhs->mis != rhs->mis) {
    return false;
  }
  return true;
}

bool
rt_interfaces__srv__SolveIK_Response__copy(
  const rt_interfaces__srv__SolveIK_Response * input,
  rt_interfaces__srv__SolveIK_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // t1
  output->t1 = input->t1;
  // t2
  output->t2 = input->t2;
  // t3
  output->t3 = input->t3;
  // success
  output->success = input->success;
  // mis
  output->mis = input->mis;
  return true;
}

rt_interfaces__srv__SolveIK_Response *
rt_interfaces__srv__SolveIK_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rt_interfaces__srv__SolveIK_Response * msg = (rt_interfaces__srv__SolveIK_Response *)allocator.allocate(sizeof(rt_interfaces__srv__SolveIK_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rt_interfaces__srv__SolveIK_Response));
  bool success = rt_interfaces__srv__SolveIK_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rt_interfaces__srv__SolveIK_Response__destroy(rt_interfaces__srv__SolveIK_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rt_interfaces__srv__SolveIK_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rt_interfaces__srv__SolveIK_Response__Sequence__init(rt_interfaces__srv__SolveIK_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rt_interfaces__srv__SolveIK_Response * data = NULL;

  if (size) {
    data = (rt_interfaces__srv__SolveIK_Response *)allocator.zero_allocate(size, sizeof(rt_interfaces__srv__SolveIK_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rt_interfaces__srv__SolveIK_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rt_interfaces__srv__SolveIK_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rt_interfaces__srv__SolveIK_Response__Sequence__fini(rt_interfaces__srv__SolveIK_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rt_interfaces__srv__SolveIK_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rt_interfaces__srv__SolveIK_Response__Sequence *
rt_interfaces__srv__SolveIK_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rt_interfaces__srv__SolveIK_Response__Sequence * array = (rt_interfaces__srv__SolveIK_Response__Sequence *)allocator.allocate(sizeof(rt_interfaces__srv__SolveIK_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rt_interfaces__srv__SolveIK_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rt_interfaces__srv__SolveIK_Response__Sequence__destroy(rt_interfaces__srv__SolveIK_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rt_interfaces__srv__SolveIK_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rt_interfaces__srv__SolveIK_Response__Sequence__are_equal(const rt_interfaces__srv__SolveIK_Response__Sequence * lhs, const rt_interfaces__srv__SolveIK_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rt_interfaces__srv__SolveIK_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rt_interfaces__srv__SolveIK_Response__Sequence__copy(
  const rt_interfaces__srv__SolveIK_Response__Sequence * input,
  rt_interfaces__srv__SolveIK_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rt_interfaces__srv__SolveIK_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rt_interfaces__srv__SolveIK_Response * data =
      (rt_interfaces__srv__SolveIK_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rt_interfaces__srv__SolveIK_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rt_interfaces__srv__SolveIK_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rt_interfaces__srv__SolveIK_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
